...
class DataLoader:
        ...
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                if "portfolio" in sheet_name.lower() or "company" in sheet_name.lower():
                    ...
                    "financial" in sheet_name.lower()
                    or "statement" in sheet_name.lower()
                ):
                    self._load_financial_data(df)
                elif (
                    "metric" in sheet_name.lower()
                    or "performance" in sheet_name.lower()
                ):
    ...
    def _load_portfolio_data(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            company_name = (
                row.get("Company Name") or row.get("Company") or row.get("Name")
            )
            if pd.isna(company_name):
                continue

            company = (
                self.db.query(Company).filter(Company.name == company_name).first()
            )
            ...
            if not company:
                company = Company(
                    ...
                    if not pd.isna(row.get("Sector"))
                    else None,
                    industry=row.get("Industry")
                    if not pd.isna(row.get("Industry"))
                    else None,
            ...
            if not pd.isna(investment_date):
                portfolio = PortfolioCompany(
                    ...
                    investment_amount=row.get("Investment Amount")
                    if not pd.isna(row.get("Investment Amount"))
                    else None,
                    current_valuation=row.get("Current Valuation")
                    if not pd.isna(row.get("Current Valuation"))
                    else None,
                    ownership_percentage=row.get("Ownership %")
                    if not pd.isna(row.get("Ownership %"))
                    else None,
                    investment_stage=row.get("Stage")
    ...
    def _load_financial_data(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            statement = FinancialStatement(
                company_id=company.id,
                statement_date=row.get("Date")
                if not pd.isna(row.get("Date"))
                else datetime.now(),
                period_type=row.get("Period")
                if not pd.isna(row.get("Period"))
                else "Annual",
                ...
                if not pd.isna(row.get("Gross Profit"))
                else None,
                operating_income=row.get("Operating Income")
                if not pd.isna(row.get("Operating Income"))
                else None,
                net_income=row.get("Net Income")
                if not pd.isna(row.get("Net Income"))
                else None,
                total_assets=row.get("Total Assets")
                if not pd.isna(row.get("Total Assets"))
                else None,
                total_liabilities=row.get("Total Liabilities")
                if not pd.isna(row.get("Total Liabilities"))
                else None,
                shareholders_equity=row.get("Equity")
                if not pd.isna(row.get("Equity"))
                else None,
            )
    ...
    def _load_performance_data(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            portfolio = (
                ...
                .filter(PortfolioCompany.company_id == company.id)
                .first()
            )
            ...
            metric = PerformanceMetric(
                ...
                customer_count=row.get("Customers")
                if not pd.isna(row.get("Customers"))
                else None,
                churn_rate=row.get("Churn Rate")
                if not pd.isna(row.get("Churn Rate"))
                else None,
            )
            self.db.add(metric)
        ...
        for ticker_symbol in tickers:
            try:
                ...
                company = (
                    ...
                    .filter(Company.ticker == ticker_symbol)
                    .first()
                )
                    ...
                    for date in quarterly_financials.columns[
                        ...
                        try:
                            statement = FinancialStatement(
                                ...
                                revenue=self._safe_get(
                                    quarterly_financials, "Total Revenue", date
                                ),
                                cost_of_revenue=self._safe_get(
                                    quarterly_financials, "Cost Of Revenue", date
                                ),
                                gross_profit=self._safe_get(
                                    quarterly_financials, "Gross Profit", date
                                ),
                                operating_income=self._safe_get(
                                    quarterly_financials, "Operating Income", date
                                ),
                                ebitda=self._safe_get(
                                    quarterly_financials, "EBITDA", date
                                ),
                                net_income=self._safe_get(
                                    quarterly_financials, "Net Income", date
                                ),
                                total_assets=self._safe_get(
                                    quarterly_balance, "Total Assets", date
                                )
                                ...
                                shareholders_equity=self._safe_get(
                                    quarterly_balance, "Stockholders Equity", date
                                )
                                if not quarterly_balance.empty
                                else None,
                                operating_cash_flow=self._safe_get(
                                    quarterly_cashflow, "Operating Cash Flow", date
                                )
                                if not quarterly_cashflow.empty
                                else None,
                                free_cash_flow=self._safe_get(
