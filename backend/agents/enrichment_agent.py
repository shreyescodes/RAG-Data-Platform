...
class EnrichmentAgent(BaseAgent):
    ...
    async def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.log_reasoning("enrichment_started", {"query": user_query})
        ...
        if any(
            ...
            for keyword in ["stock", "ticker", "market", "price", "yahoo"]
        ):
            ...
            if ticker:
                ...
                if market_data:
                    ...
                    self.log_reasoning(
                        "yahoo_finance_enrichment",
                        {"ticker": ticker, "data_fetched": True},
                    )

        # Check if SEC EDGAR data is needed
        if any(
            keyword in user_query
            for keyword in ["sec", "edgar", "filing", "10-k", "10-q"]
        ):
            ...
            if company_cik:
                ...
                if sec_data:
                    ...
                    self.log_reasoning(
                        ...
                        {"company": company_cik, "data_fetched": True},
                    )

        if not enriched_data:
            self.log_reasoning(
                "no_enrichment_needed", "Query does not require external data"
            )
    ...
    async def _fetch_sec_edgar(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                params = {"company": company_identifier, "output": "json"}
